window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function snapshotOf (sessionId) {
			return api("/api/audit/snapshot?session=" + encodeURIComponent(sessionId), { cache: "no-store" });
		}
		function requestNow (sessionId, final) {
			return api("/api/audit/request-now", {
				method: "POST",
		...
		function resumeOf (sessionId) {
			return api("/api/audit/resume", {
				method: "POST",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({ sessionId: sessionId })
			});
		}
		function acceptOf (sessionId, auditId, editedText) {
			return api("/api/audit/accept", {
				method: "POST",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({ sessionId: sessionId, auditId: auditId, editedText: editedText })
			});
		}
		function historyOf (sessionId) {
			return api("/api/audit/history?session=" + encodeURIComponent(sessionId), { cache: "no-store" });
		}
		...
		var STYLES = {
			...
			editor: { boxSizing: "border-box", margin: "4px 8px 0", padding: "8px", border: "1px solid var(--dsw-alias-border-l1, #444)", borderRadius: "10px" },
			textarea: { boxSizing: "border-box", width: "100%", minHeight: "96px", resize: "vertical", border: "1px solid var(--dsw-alias-border-l1, #444)", borderRadius: "8px", padding: "8px", color: "var(--dsw-alias-label-primary, #ddd)", background: "var(--dsw-alias-background-l2, transparent)", font: "inherit" }
		};
		...
		function AuditDock (props) {
			...
			useEffect(function () {
				...
				var source = new EventSource(base + "/api/audit/watch?session=" + encodeURIComponent(sessionId));
				...
				source.addEventListener("audit", onEvent);
				return function () { alive = false; source.close(); };
			}, [sessionId]);
			var current = state ?? view ?? null;
			var draftApproval = current && current.pendingApproval ? current.pendingApproval : null;
			var draftRemediation = current && current.remediation ? current.remediation : null;
			var draftKey = draftApproval ? String(draftApproval.auditId) + ":" + String(draftApproval.status) + ":" + String(draftRemediation && draftRemediation.phase || "") : "";
			var draftSeed = draftRemediation && draftRemediation.instruction ? draftRemediation.instruction : draftApproval && draftApproval.editableText ? draftApproval.editableText : "";
			useEffect(function () {
				setEditing(false);
				setDraft(draftSeed);
			}, [sessionId, draftKey]);
			...
			var label = remediation && ["queued", "running", "verifying"].indexOf(remediation.phase) >= 0 ? "REPAIR" : current.paused ? "PAUSED" : current.lastVerdict === undefined ? "audit" : String(current.lastVerdict).toUpperCase();
			var deliveryText = remediation && remediation.delivery === "next-step" ? "after current tool call" : remediation ? "immediate" : "";
			var stateText = (current.reviewer || "codex") + " · step " + current.completedSteps + " · audits " + current.auditCount + " · " + (remediation ? "repair " + remediation.phase + " · " + deliveryText : current.paused ? "paused: " + current.pauseReason : approval ? approval.verdict + " review awaiting approval; agent continues" : "failures " + current.failureCount);
			...
			return h("div", { style: STYLES.root, "data-audit-bar": true },
				h("div", { style: Object.assign({}, STYLES.bar, { borderColor: color }) },
					...
					approvalAction ? h("button", { style: STYLES.button, disabled: pending, onClick: function () { run(function () { return onAccept ? onAccept(approvalAction.auditId, undefined) : undefined; }); } }, retryable ? "retry repair" : approvalAction.verdict === "critical" ? "execute now" : "execute after tool") : null,
					approvalAction ? h("button", { style: STYLES.button, disabled: pending, onClick: function () { setEditing(!editing); } }, editing ? "close editor" : "edit repair") : null,
				...
				!editing || !approvalAction ? null : h("div", { style: STYLES.editor },
					h("textarea", { style: STYLES.textarea, value: draft, onChange: function (event) { setDraft(event.target.value); }, "aria-label": "Edit Audit remediation" }),
					h("div", { style: { display: "flex", justifyContent: "flex-end", gap: "8px", paddingTop: "6px" } },
						h("button", { style: STYLES.button, disabled: pending || draft.trim() === "", onClick: function () { run(function () { return onAccept ? onAccept(approvalAction.auditId, draft) : undefined; }); } }, approvalAction.verdict === "critical" ? "execute edited now" : "execute edited after tool"),
						h("button", { style: STYLES.button, disabled: pending, onClick: function () { setDraft(draftSeed); setEditing(false); } }, "cancel")
					)
				),
		...
		/**
		 * Client plugin body: the audit dock footer between Todo (order 0)
				...
				return ctx.slots.register({
					...
					inject: function (sessionId) {
						return {
							...
							onAccept: function (auditId, editedText) { return acceptOf(String(sessionId), auditId, editedText); },
		...
		exports.AuditDock = AuditDock;
		exports.apply = apply;
		exports.inject = ["slots", "locale"];
		exports.AUDIT_DOCK_ORDER = 5;
