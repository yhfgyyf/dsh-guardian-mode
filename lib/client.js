window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ── the standalone Remote API (snapshot/watch/requestNow/resume) ───────
		...
		function GuardianDock (props) {
			...
			var label = current.paused ? "PAUSED" : current.lastVerdict === undefined ? "guardian" : String(current.lastVerdict).toUpperCase();
			var stateText = "step " + current.completedSteps + " · audits " + current.auditCount + " · " + (current.paused ? "paused: " + current.pauseReason : "failures " + current.failureCount);
			...
			return h("div", { style: STYLES.root, "data-guardian-bar": true },
				h("div", { style: Object.assign({}, STYLES.bar, { borderColor: color }) },
					...
					current.paused ? h("button", { style: STYLES.button, disabled: pending, onClick: function () { run(function () { return onResume ? onResume() : undefined; }); } }, "resume") : null
				),
