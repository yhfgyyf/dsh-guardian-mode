/**
 * `/guardian status|now|history|resume` — the human-facing face of the mode.
 * Registered on the host commands registry so both the Web input trigger and
 * the TUI slash menu see it, exactly like /goal.
 */
export const USAGE = 'Usage: /guardian [status|now|history|resume]'
...
export function renderStatus (view) {
  ...
  if (view.paused) lines.push('', 'Resume: /guardian resume')
...
export function registerGuardianCommand (ctx) {
  ...
  return commands.register({
    name: 'guardian',
    description: 'Guardian mode: status, now (audit), history, resume',
    input: { hint: 'status|now|history|resume' },
