...
import { REVIEWER_MODEL_DEFAULTS, CodexClient, CodexCompanion } from './codex.js'
...
export const REVIEWER_DEFAULTS = Object.freeze({
  ...
  models: REVIEWER_MODEL_DEFAULTS
})
...
export function resolveReviewerOptions (config = {}) {
  ...
  if (!REVIEWER_TYPES.includes(type)) throw new ReviewerError(`unsupported audit reviewer: ${String(type)}`, 'REVIEWER_UNSUPPORTED')
  return {
    type,
    ...
    models: {
      summarizer: copyModel(REVIEWER_MODEL_DEFAULTS.summarizer, config.models?.summarizer ?? config.models?.luna),
      auditor: copyModel(REVIEWER_MODEL_DEFAULTS.auditor, config.models?.auditor ?? config.models?.sol)
    }
  }
}
...
function validateClaudeArgs (args) {
  ...
  if (unsafe !== undefined) {
    throw new ReviewerError(`claudeArgs cannot override Audit-managed flag ${unsafe}`, 'CLAUDE_ARGS_CONFLICT')
  }
}
  ...
  async runTurn ({ threadId, text, model, effort, provider, outputSchema, baseInstructions }) {
    ...
    try {
      for await (const chunk of this.llm.stream({
        ...
        messages: [message('user', framed, { kind: 'plugin', plugin: 'dsh-audit-mode' })],
