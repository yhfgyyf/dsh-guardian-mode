...
export class CodexCompanion {
  ...
  async ensureThreads (state, { parentThreads } = {}) {
    ...
    for (const role of ['luna', 'sol']) {
      ...
      let id
      const baseInstructions = role === 'luna' ? LUNA_BASE_INSTRUCTIONS : SOL_BASE_INSTRUCTIONS
      if (current !== undefined) id = await this.client.resumeThread({ threadId: current, cwd: this.cwd, model, baseInstructions })
      else if (parentThreads?.[role] !== undefined) id = await this.client.forkThread({ threadId: parentThreads[role], cwd: this.cwd, model, baseInstructions })
  ...
  async runSummary (state, text, outputSchema) {
    const reply = await this.client.runTurn({
      ...
      provider: this.models.luna.provider,
      cwd: this.cwd,
      outputSchema,
      baseInstructions: LUNA_BASE_INSTRUCTIONS
    })
    if (reply.threadId !== undefined) state.threads.luna = reply.threadId
    return reply
  }
  ...
  async runAudit (state, text, outputSchema) {
    const reply = await this.client.runTurn({
      ...
      provider: this.models.sol.provider,
      cwd: this.cwd,
      outputSchema,
      baseInstructions: SOL_BASE_INSTRUCTIONS
    })
    if (reply.threadId !== undefined) state.threads.sol = reply.threadId
    return reply
  }
