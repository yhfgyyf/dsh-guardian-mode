...
import { dispatchRemediation } from './core.js'
import { AUDIT_PRESET_ID, isAuditPresetId } from './capability.js'
import { AUDIT_SERVICE } from './invariant.js'
import { SidecarStore } from './sidecar.js'
import { redactText } from './trace.js'

const AUDIT_PLUGIN = 'dsh-audit-mode'
const RETIRED_PLUGIN = `dsh-${['guard', 'ian'].join('')}-mode`
...
const SKILL_ROOT = new URL('../presets/audit/skills/', import.meta.url)

function auditMessage (text) {
  return createUserMessage({
    content: [{ type: 'text', text }],
    source: { kind: 'plugin', plugin: AUDIT_PLUGIN }
  })
}

function isRemediationMessage (message) {
  return message?.source?.kind === 'plugin' && [AUDIT_PLUGIN, RETIRED_PLUGIN].includes(message.source.plugin)
}

function capabilityLeasePrompt (remediation, skills) {
  return [
    `<audit-capability-lease id="${String(remediation.id)}">`,
    ...
    'All calls remain subject to DSH permission, approval, safety, scheduling, and audit policy.',
...
export class AuditService extends Service {
  ...
  constructor (ctx, config = {}) {
    ...
    void this.store.migrateRetired().catch((error) => this._warn(error))
    ...
    ctx.on('session/created', (session) => {
      if (this._auditedSession(session)) void this._withLock(session.id, () => this._activate(session)).catch((error) => this._warn(error))
    })
    ctx.on('agent-preset/selected', (sessionId, preset) => {
      if (!isAuditPresetId(preset)) return
    ...
    ctx.on('session/event', (session, event) => {
      if (!this._auditedSession(session)) return
    ...
    ctx.on('agent/inbox/claimed', ({ agent, message }) => {
      if (!this._auditedAgent(agent) || !isRemediationMessage(message)) return
    ...
    ctx.on('agent/pre-step', async ({ agent, messages }, next) => {
      if (!this._auditedAgent(agent)) return await next()
      const id = String(agent.session.id)
      const blocked = await this._withLock(id, async () => {
        ...
        if (await engine.due(id)) {
          const result = await engine.audit(id, sessionEvents(agent.session), {
    ...
    ctx.on('agent/turn-stopping', async ({ agent }) => {
      if (!this._auditedAgent(agent) || !this._isTaskEnding(agent)) return
      const id = String(agent.session.id)
    ...
    }, 'dsh-audit-mode.shutdown()')
  }

  _warn (error) { this.ctx.logger?.warn?.(error instanceof Error ? error : new Error(String(error))) }

  _auditedSession (session) {
    return this.options.enabled && isAuditPresetId(selectedPreset(session))
  }

  _auditedAgent (agent) {
    if (!this.options.enabled) return false
    const roster = this.ctx.get('agentPresets')
    return roster?.composedPreset?.(agent.ctx) === AUDIT_PRESET_ID || this._auditedSession(agent.session)
  }
  ...
  async _enableRepairRuntime (agent) {
    ...
    try {
      ...
      if (registry === undefined) throw new Error('audit: the active preset has no scoped skill registry')
  ...
  _queueContinuation (agent) {
    agent.followup(auditMessage([
      '<audit-remediation-complete>',
      'Audit verified the accepted remediation. Continue the original user task from the preserved conversation and current workspace state.',
  ...
  async _verifyRemediation (sessionId, turnReason) {
    ...
    this._emit(sessionId)
    const result = await engine.audit(sessionId, sessionEvents(agent.session), {
      objective: this._objective(agent),
      reason: 'remediation',
      force: true
    })
  ...
  _newEngine (sessionId, cwd) {
    ...
    const engine = new AuditEngine(this.store, companion, { logger: (line) => this.ctx.logger?.info?.(line) })
  ...
  async requestNow (sessionId, { final = false } = {}) {
    return await this._withLock(sessionId, async () => {
      const agent = this.ctx.agents.get(sessionId)
      let events = agent === undefined ? undefined : sessionEvents(agent.session)
      ...
      if (engine === undefined) {
        const state = await this.store.load(sessionId)
        if (state === undefined) throw new Error('audit is not active for this session')
        if (agent !== undefined) {
      ...
      if (result.state?.paused && agent !== undefined) this._haltAgent(agent, 'audit critical review')
      this._emit(sessionId)
      return result.state
    })
  }
  ...
  async resume (sessionId) {
    return await this._withLock(sessionId, async () => {
      ...
      if (state?.pendingApproval?.status === 'pending' && state.pendingApproval.verdict === 'critical') {
        throw new Error('audit critical review is awaiting approval; accept it before resuming')
      }
      if (state?.remediation !== undefined && state.remediation.phase !== 'completed') {
        throw new Error('audit remediation has not completed')
      }
  ...
  async accept (sessionId, auditId, editedText) {
    return await this._withLock(sessionId, async () => {
      const agent = this.ctx.agents.get(sessionId)
      if (agent === undefined) throw new Error('audit approval requires a live DSH session')
      const engine = await this._activate(agent.session)
      const before = await engine.get(sessionId)
      const verdict = before?.pendingApproval?.verdict ?? before?.remediation?.verdict
      const delivery = verdict === 'warning' && agent.status === 'running' ? 'next-step' : 'next-turn'
      let skills = []
      ...
      let view
      try {
        view = await engine.accept(sessionId, auditId, editedText, delivery)
      } catch (error) {
      ...
      try {
        const message = auditMessage(tail.join('\n\n'))
        dispatchRemediation(agent, message, view.remediation?.delivery, () => {
          // Critical already paused the task. Abort the active turn at its safe
          // boundary and wake an immediate remediation turn.
          this._haltAgent(agent, 'audit remediation approved')
        })
      } catch (error) {
  ...
  registerCapability (name, handler) {
    if (this.dynamicCapabilities.has(name)) throw new Error(`audit capability already registered: ${name}`)
  ...
  async invokeCapability (name, input, execution) {
    ...
    if (handler === undefined) throw new Error(`unknown audit capability: ${name}`)
