...
function textStats (data) {
  ...
  for (const block of content) {
    ...
    else if (block?.type === 'image' || block?.type === 'image-url' || block?.type === 'image_url') images += 1
  }
  return { chars, images }
}
  ...
  switch (e.type) {
    ...
    case 'tool/result':
      return {
        ...
        contentChars: Array.isArray(data.message?.content) ? data.message.content.reduce((n, block) => n + (typeof block?.text === 'string' ? block.text.length : 0), 0) : 0,
