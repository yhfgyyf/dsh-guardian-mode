...
function normalizeRemediationText (value, fallback) {
  ...
  if (text === '') throw new Error('guardian: remediation text cannot be empty')
  if (text.length > MAX_REMEDIATION_TEXT_CHARS) {
    throw new Error(`guardian: remediation text exceeds ${MAX_REMEDIATION_TEXT_CHARS} characters`)
  }
  return text
}
...
function remediationPrompt (approval, text, edited) {
  return [
    ...
    `The user ${edited ? 'edited and accepted' : 'accepted'} Guardian audit ${approval.auditId} (${approval.verdict}).`,
    'Treat the following text as an approved remediation request, not as a new objective.',
    '<guardian-approved-instructions>',
    text,
    '</guardian-approved-instructions>',
    '</guardian-remediation>'
  ].join('\n')
