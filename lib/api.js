import { AUDIT_API_BASE } from './invariant.js'

/**
 * Audit Remote API: snapshot / watch / requestNow / accept / resume over the host
 * webserver (third-party routes never touch the fixed RPC map).
 *   GET  /api/audit/snapshot?session=<id>   ->  JSON view
 *   GET  /api/audit/watch?session=<id>      ->  text/event-stream (SSE)
 *   POST /api/audit/request-now             ->  {sessionId, reason?, final?}
...
async function readBody (req) {
  ...
  for await (const chunk of req) {
    size += chunk.length
    if (size > 32 * 1024) throw new Error('audit request body is too large')
    chunks.push(chunk)
  }
...
export function registerAuditApi (ctx, service) {
  ...
  add({
    ...
    handler: async (req, res) => {
      ...
      const send = (event) => res.write('event: audit' + String.fromCharCode(10) + 'data: ' + JSON.stringify(event) + String.fromCharCode(10, 10))
  ...
  add({
    kind: 'exact',
    path: AUDIT_API_BASE + '/request-now',
    handler: async (req, res) => {
  ...
  add({
    kind: 'exact',
    path: AUDIT_API_BASE + '/accept',
    handler: async (req, res) => {
      if (req.method !== 'POST') return sendJson(res, 405, { error: 'method not allowed' })
      try {
        const body = await readBody(req)
        if (body.sessionId === undefined) return sendJson(res, 400, { error: 'missing sessionId' })
        sendJson(res, 200, { ok: true, view: await service.accept(String(body.sessionId), body.auditId, body.editedText) })
      } catch (error) {
        sendJson(res, 500, { error: String(error) })
      }
    }
