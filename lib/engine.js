...
function auditRequestSemantics (reason, final) {
  ...
  return `\nAUDIT REQUEST SEMANTICS: This prompt is the active ${kind} audit triggered after trace capture. Guardian verdicts and feedback are intentionally sidecar/UI-only and never enter the main DSH trace. Therefore the trace cannot contain this audit's future verdict and need not contain prior Guardian stdout or status. Do not report absent Guardian output as missing task evidence or as a finding; evaluate the supplied task evidence and produce the current verdict now.`
}
