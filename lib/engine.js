...
function reviewerInfo (companion) {
  return {
    ...
    models: { luna: companion.models.luna.model, sol: companion.models.sol.model }
  }
}
...
function auditPrompt ({ sequence, reason, summary, objective, fullAlignment, final, includeObjective = false }) {
...
export class GuardianEngine {
  ...
  async attach (sessionId, { parentThreads, parentReviewer } = {}) {
    ...
    if (storedReviewer !== undefined && storedReviewer !== reviewer) state.threads = { luna: undefined, sol: undefined }
  ...
  async audit (sessionId, events, { objective, reason = 'auto', final = false, force = false } = {}) {
    ...
    if (failure === undefined) {
      try {
        const reply = await this.companion.runAudit(state, auditPrompt({ sequence, reason, summary, objective, fullAlignment, final, includeObjective: statelessReviewer }), AUDIT_SCHEMA)
        rawAudit = reply.text
  ...
  async accept (sessionId, auditId) {
    const state = await this.get(sessionId)
    if (state === undefined) throw new Error(`guardian: session ${sessionId} is not attached`)
    acceptPendingApproval(state, auditId, this.now())
    return this.view(await this.store.save(sessionId, state))
  }
