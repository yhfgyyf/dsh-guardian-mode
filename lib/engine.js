...
import { AUDIT_CAPABILITY } from './capability.js'
...
export const BOUNDARY_RULES = Object.freeze([
  ...
  'Do not bypass permissions, approval prompts, safety gates, or the Audit pause.',
...
export function extractJson (text) {
  if (typeof text !== 'string') throw new TypeError('audit: reviewer reply is not text')
  const start = text.indexOf('{')
  if (start < 0) throw new TypeError('audit: reviewer reply has no JSON object')
  let depth = 0
  let inString = false
  let escaped = false
  ...
  throw new TypeError('audit: reviewer reply has unbalanced JSON')
}
...
export function coerceVerdict (value) {
  ...
  throw new TypeError('audit: invalid verdict ' + JSON.stringify(value))
}
...
function auditRequestSemantics (reason, final) {
  ...
  return `\nAUDIT REQUEST SEMANTICS: This prompt is the active ${kind} audit triggered after trace capture. Unaccepted Audit verdicts and feedback are sidecar/UI-only. A prior finding may appear in the main DSH trace only inside a <audit-remediation> tail message after explicit user acceptance; that message is an approved repair request, not independent task evidence. The trace cannot contain this audit's future verdict and need not contain Audit stdout or status. Do not report absent Audit output as missing task evidence; evaluate the supplied task and remediation evidence and produce the current verdict now.`
}
...
export class AuditEngine {
  ...
  _log (message) { this.logger('[audit] ' + message) }
  ...
  async audit (sessionId, events, { objective, reason = 'auto', final = false, force = false } = {}) {
    ...
    if (state === undefined) throw new Error(`audit: session ${sessionId} is not attached`)
